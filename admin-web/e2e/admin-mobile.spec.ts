...
test('phone navigation is a closable, keyboard-contained drawer', async ({ page }) => {
  ...
  await expect(drawer).toHaveCSS('background-color', 'rgb(243, 240, 232)')
  await expect(drawer).toHaveCSS('background-image', 'none')
  await expect(drawer.getByRole('button', { name: '关闭菜单', exact: true })).toHaveCSS('color', 'rgb(23, 70, 61)')
...
test('desktop retains its sidebar, header and warm colors', async ({ page }) => {
  ...
  await expect(page.locator('.sidebar')).toHaveCSS('background-color', 'rgb(243, 240, 232)')
  await expect(page.locator('.sidebar')).toHaveCSS('background-image', 'none')
  await expect(page.locator('.sidebar-brand__copy')).toHaveCSS('color', 'rgb(23, 70, 61)')
  const active = page.getByRole('navigation', { name: '管理后台主导航' }).getByRole('link', { name: '工作台', exact: true })
  await expect(active).toHaveCSS('background-color', 'rgb(255, 253, 248)')
  await expect(active).toHaveCSS('color', 'rgb(16, 58, 51)')
  expect(await page.evaluate('getComputedStyle(document.querySelector(".sidebar .nav-link.active"), "::before").backgroundColor')).toBe('rgb(173, 137, 80)')
  const inactive = page.getByRole('navigation', { name: '管理后台主导航' }).getByRole('link', { name: '使用指南', exact: true })
  await expect(inactive).toHaveCSS('color', 'rgb(23, 70, 61)')
  await inactive.hover()
  await expect(inactive).toHaveCSS('background-color', 'rgb(248, 245, 237)')
  await expect(inactive).toHaveCSS('color', 'rgb(16, 58, 51)')
  await active.focus()
  await page.keyboard.press('Tab')
  await expect(inactive).toBeFocused()
  await expect(inactive).toHaveCSS('outline-color', 'rgb(173, 137, 80)')
