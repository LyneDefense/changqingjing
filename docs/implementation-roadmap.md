...
| N1 | 三工程初始化 | 三个工程可在 macOS 启动和构建 | [ ] |
| N2 | PostgreSQL 与后端基础 | 迁移、错误响应、两套安全链和健康检查可运行 | [ ] |
...
- [ ] `backend` 在 JDK 18 下完成编译、测试和启动。
- [ ] 本地 PostgreSQL 通过 Docker Compose 启动，后端能连接。
- [ ] `admin-web` 可运行、构建并通过 `/admin/` base 预览。
- [ ] `miniprogram` 可构建并出现在微信开发者工具模拟器中。
- [ ] 三个工程分别拥有依赖锁定或 Wrapper、环境示例和基础 README。
- [ ] 仓库不存在 AppSecret、数据库密码、COS 永久密钥等真实秘密。
...
- [ ] 空数据库可以从零执行全部迁移，重复启动不重复建表。
- [ ] PostgreSQL 集成测试使用真实 PostgreSQL，不用 H2 代替。
