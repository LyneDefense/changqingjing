...
| 当前状态 | N0～N9 功能实现完成；N10 自动化发布门禁通过；N11～N13 部署、发布和运维工具已完成本地生产式验证。尚待真实域名／Ubuntu 执行、Android 与微信真机验收、小程序提审和正式发布。 |
...
| 节点 | 名称 | 核心可验收结果 | 状态 |
| --- | --- | --- | --- |
| N0 | 范围与外部输入确认 | 第一版范围、字段和外部账号清单冻结 | 已完成 |
| N1 | 三工程初始化 | 三个工程可在 macOS 启动和构建 | 已完成 |
| N2 | PostgreSQL 与后端基础 | 迁移、错误响应、两套安全链和健康检查可运行 | 已完成 |
| N3 | 后台登录与人员管理 | 管理员可登录并管理后台人员 | 已完成 |
| N4 | 内容发布与公开首页闭环 | 后台发布公司介绍，小程序无需登录即可看到 | 已完成 |
| N5 | 腾讯云 COS 媒体闭环 | 后台可安全上传并发布图片／视频，小程序可读取 | 实现完成；Android 真机待验 |
| N6 | 景区、地图选点与导航 | 后台选点改名，小程序准确展示并导航 | 实现完成；双端真机导航待验 |
| N7 | 微信登录与注册用户 | 指定入口一键登录，首次注册、后续恢复，后台可查看用户 | 实现完成；真实微信授权待验 |
| N8 | 会员福利产品 | 登录后浏览分类、搜索、列表和纯展示详情 | 已完成 |
| N9 | 合作权益与个人中心 | 开放内容完成，所有未开放入口统一提示 | 已完成 |
| N10 | 全量联调与质量验收 | 产品 AC、权限、兼容、弱网和安全检查通过 | 自动化通过；外部真机项待验 |
| N11 | 测试环境一键部署 | 单域名、Docker、Nginx、HTTPS、续签和部署脚本通过 | 工具完成；真实服务器待执行 |
| N12 | 正式发布 | 正式后端、管理后台和小程序完成上线验证 | 制品工具完成；部署／提审待执行 |
| N13 | 上线运维 | 监控、备份、证书、回滚和内容维护进入常态 | 工具完成；上线后启用 |

“工具完成”表示代码、配置和本地生产式演练已经通过，不代表云服务器、域名、证书或微信平台已经发生变更。依赖外部账号和真实设备的项目仍按本表明确保留，不能用本地测试结果冒充上线结果。
...
- [x] 产品待定项按产品文档默认决策落地，未定义的交易／会员资产功能明确不在首版实现。
- [x] 页面结构按产品参考与运营反馈冻结，后台按业务板块拆分。
- [x] OpenAPI 资源与数据库对象已实现并固化为可生成契约。
- [x] 外部账号、素材及生产信息的最晚提供节点已在第 18 节明确。
...
- [x] 未认证访问受保护接口返回 401，无权限返回 403，未知管理接口默认拒绝。
- [x] 后台 Cookie 不能访问小程序受保护接口，小程序 token 不能访问管理接口。
- [x] 日志不包含密码、token、手机号或微信临时 code。
...
- [x] 初始管理员可安全创建并首次登录。
- [x] 管理员能管理人员，运营无法访问人员菜单或接口。
- [x] 停用人员、重置密码、角色变更后旧会话失效。
- [x] 不能停用或降权最后一个有效管理员。
- [x] 登录失败不泄漏账号是否存在；连续失败受到限流。
...
- 完成公司介绍业务板块编辑、增加／删除／排序、保存草稿、预览、发布和下架页面。
...
- [x] 后台保存草稿后，小程序仍展示旧版本。
- [x] 后台发布后，小程序刷新可见新版本，无需重新发布小程序代码。
- [x] 下架后公开接口和旧链接不再返回正文。
- [x] 两人并发编辑时后保存者收到冲突提示，不静默覆盖。
- [x] 后台预览需要后台权限，游客无法读取草稿。
...
- 完成首页宣传视频列表、状态筛选、新增／删除、封面、播放预览和发布；完成公司介绍图片排序、预览和发布。
...
- [x] COS CORS 只允许实际管理后台来源，临时凭证只允许指定 Object Key。
- [x] 伪造扩展名、超限文件和未完成上传不能进入 READY 或发布。
- [x] 视频支持 Range 请求并返回正确的 `206 Partial Content`。
- [x] 视频在 iOS 真机正常播放、拖动、暂停和全屏。
- [ ] 视频在 Android 真机正常播放、拖动、暂停和全屏。
- [x] 替换媒体失败时，线上原视频／图片保持可用。
- [x] 清理任务不会删除仍被引用的文件。
...
N5 完成。地图选点联调时提供开发用腾讯位置服务 key，并确认地图账号可使用所需网页能力；不使用导航时可暂不配置。

### 小程序后端

- 实现景区分页列表、详情和浏览次数接口。
- 返回已发布版本的标题、图文和开放状态；配置了导航地点时再返回展示名称、详细地址和 GCJ-02 坐标。
...
- 发布前检查媒体状态；已选择导航地点时检查坐标和展示名称完整性，未选点不阻止发布。
...
- [x] 游客浏览首页、景区和合作权益时无登录弹窗、无用户记录。
- [x] 首次授权只创建一个用户，重复请求和再次登录不产生重复账号。
- [ ] 会员福利入口与个人中心两条登录路径行为符合产品规则。
- [ ] 拒绝授权回首页；网络失败留在登录流程并可重试，不误判为取消。
- [x] 前后端日志没有完整手机号、微信临时 code、AppSecret 或业务 token。
- [x] 后台只能由授权人员查看注册用户。
...
- 产品支持上传多张候选图片并指定列表封面；名称、简介必填，详情多图、图文内容和规格均为选填。
...
- [x] 未登录不能通过页面或直接接口获得产品正文。
- [x] 登录后分类、搜索、分页和详情结果一致。
- [x] 产品上下架、分类停用和旧链接行为符合规则。
- [x] 详情只展示内容，没有交易相关入口或后端数据结构。
...
- 完成合作权益开放内容页面。
- 分公司方案、会员体系在同一页面内切换，以“敬请期待”替换核心收益区域，并继续显示合作价值总结。
- 完成个人中心未登录和已登录状态。
- 会员空间、成为会员、我的订单、充值记录、地址管理、我的推荐点击后统一提示“敬请期待”，不在服务列表内重复显示状态，不触发登录或进入空页面。
...
- [x] 数据库备份在隔离的生产式 Compose 中实际生成，并完成恢复到临时数据库的演练；真实服务器上线后需再次执行。
- [ ] 测试环境端到端验收通过。
