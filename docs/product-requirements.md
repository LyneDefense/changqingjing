...
| 福利产品 | 列表、多张候选图片中指定一张列表封面、名称、简介、可选图文详情 | 已确认 |
...
- 个人中心登录后展示“退出登录”入口。用户确认退出后撤销当前服务端会话并清理本地登录状态，不立即自动登录回来。
...
产品列表：展示已上架产品；每条使用一张列表封面、名称和简短介绍，按后台设置排序。后台可为产品上传多张图片并从中指定一张列表封面；进入详情后，顶部图片区域以选定封面为首图，可左右滑动查看全部产品图片。采用分页加载，支持加载中、加载失败重试和没有更多内容状态。

搜索与分类为参考功能及补充设计：默认“全部”，有有效分类时展示其他分类；按产品名称搜索，并与当前分类一起筛选。无结果时显示清晰提示并允许清空搜索条件。

产品详情：展示名称、简短介绍，以及按需填写的详细文字、多张详情图片和材质／规格等信息。列表封面与详情图片分别维护；详细文字、详情图片和规格均可留空。
...
| 状态 | 展示与操作 |
| --- | --- |
| 未登录 | 默认品牌头像、“登录后完善个人资料”提示、微信手机号一键登录按钮 |
| 已登录 | 头像、显示昵称、脱敏手机号，以及确认启用的用户信息字段 |
| 登录处理中 | 显示处理状态，防止重复提交 |
| 登录失败 | 提示原因或重试，不展示为已登录 |

已登录时在“账户信息”区域提供低干扰的“退出登录”入口。退出前二次确认，确认后撤销当前服务端会话并切换为未登录状态。

参考图包含会员等级、余额、功德、会员号，但这些字段的业务含义尚未定义。首版统一用“—”占位，不把游客显示为普通会员，也不伪造余额、功德等账户资产。字段是否全部保留、会员号是否生成及规则，见待定项。

“成为会员、我的订单、充值记录、地址管理、我的推荐”入口均为暂未开放。列表只展示服务名称和进入箭头，不在每一行重复显示状态；点击后通过弹窗显示“敬请期待”。不建设对应表单、数据列表或后台业务。
...
- 小程序顶部固定展示“常清静文旅投”和“业务架构与合作权利”，后台不提供页面标题、简介编辑项。
- 维护收益板块的名称、说明、图标及排序；图标只能从系统图标库选择，新增分类默认使用“通用”图标，不要求运营人员填写图标字符或文件名。
- 历史数据中的数字、Emoji 等旧图标值按分类名称自动匹配系统图标，无法匹配时使用“通用”图标，小程序不直接显示原始字符。
- 维护合作价值的图文内容。
- 预览后发布，小程序展示已发布版本。
...
| 管理操作 | 冻结、解冻、删除；仅管理员，操作前核对身份并确认 |

手机号默认脱敏；是否允许指定人员查看完整手机号待确认。列表不直接展示微信身份凭证、登录 token 等技术信息。

管理员可冻结、解冻和删除注册用户，运营人员不可操作。冻结保留资料并阻止恢复和新登录；冻结、解冻都清除旧登录，解冻后需要重新登录。删除移除个人资料、手机号和微信绑定，并使所有登录失效，后台无法撤销；用户可重新授权注册为新账号。冻结、解冻与删除前核对用户身份并确认，服务端校验版本并记录操作。删除不影响其他用户和已发布业务内容。当前仍不提供用户导出、批量营销、余额修改、功德修改或会员升级。
...
| Q-03 | 首版头像、昵称是否允许编辑，是否需要账号注销入口 | 默认头像与昵称；主动退出已确认，资料编辑与账号注销流程仍需另行明确 |
