...
- 主动退出的服务端撤销接口可以预留，但产品未确定退出入口前不增加新的用户菜单。
...
`kind` 固定为 `HOME_VIDEO`、`COMPANY`、`SCENIC`、`PRODUCT`、`PRODUCT_CATEGORY`、`COOPERATION`。首页视频、公司介绍和合作权益通过业务键保证各只有一个当前条目；其内部可以包含多项展示内容。分类也走版本与发布流程，避免统一草稿规则只对部分内容生效。
...
2. 确认待发布版本仍为目标草稿，验证类型、必填项、地图位置、资源 READY 状态和分类引用。
3. 把 `published_revision_id` 指向该版本，设可见状态为 PUBLISHED；首次发布时设置首次发布时间。
4. 记录发布审计并提交；提交成功才向客户端报告成功。

下架：在事务内设置不可见并记录审计，保留已发布版本指针便于后台核对。小程序始终检查条目可见状态，旧链接返回 404。重新上架仍走发布校验。
...
正式接口返回已发布版本中的目的地坐标及 `displayName`。小程序用该坐标调用 `wx.openLocation` 或 Taro 对应接口，`name` 传自定义展示名，`address` 传详细地址；不根据显示名称重新地理编码。
...
- 页面模块包含人员、注册用户、首页、景区、产品分类、产品、合作权益。
...
- 上传组件展示进度、VERIFYING、READY 与 FAILED，资源未就绪时不可发布。
